...
- 当前治理页已覆盖最小 `governance-overview + waiting/list + collaboration-dashboard + handoffs/list + recent runs`；`mailbox / work-items / agents/projects 真实控制面 / 更完整值班建议` 仍待继续迁入。

下一步应优先继续迁入剩余真实 `http-platform` 路由与独立前端页面，包括 `mailbox / work-items` 这批平台协作读写面，以及把本地 token 存储与平台服务端鉴权事实继续收口到同一控制面。
