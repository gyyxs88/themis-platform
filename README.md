...
- 当前 `platform-main` 已具备真实部署入口、最小 `Web Access + Bearer` 鉴权语义，以及 `mysql` driver 下的 `shared cache SQLite + MySQL shared snapshot store` wiring；平台写动作与 scheduler tick 现在都会先更新本地服务态，再按变更类型决定是否把 shared snapshot flush 到本地 cache 与 MySQL，flush 失败时也会恢复本地 shared cache 和内存态。`nodes/heartbeat` 与已有 `nodeId` 的重复 `nodes/register` 属于高频存活状态，只写本地 `runtime-state.json`，不再全量覆盖 MySQL shared snapshot，避免在 `ROW + binlog_row_image=FULL` 下把 heartbeat 放大成持续 binlog 写入。
