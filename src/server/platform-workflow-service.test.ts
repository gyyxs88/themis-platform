...
test("PlatformWorkflowService 会提供 work-items 与 mailbox 最小读写闭环", () => {
  ...
  const dispatchPayload = workflowService.dispatchWorkItem({
    ...
    workItem: {
      ...
      workspacePolicySnapshot: {
        workspacePath: "/srv/platform-frontend",
      },
  ...
  assert.equal(dispatchPayload.workItem.sourcePrincipalId, "principal-platform-owner");
  assert.deepEqual(dispatchPayload.workItem.contextPacket, { ticket: "ops-1" });
  assert.deepEqual(dispatchPayload.workItem.workspacePolicySnapshot, { workspacePath: "/srv/platform-frontend" });
  assert.deepEqual(dispatchPayload.workItem.runtimeProfileSnapshot, { model: "gpt-5.5" });
  assert.equal(dispatchPayload.workItem.scheduledAt, "2026-04-14T12:00:00.000Z");
  assert.equal(dispatchPayload.targetAgent.agentId, frontendAgent.agentId);
