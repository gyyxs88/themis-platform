...
test("PlatformControlPlaneService 会提供 agents 与 projects 最小控制面闭环", () => {
  ...
  assert.equal(detail?.runtimeProfile.model, "gpt-5.4");
  assert.equal(detail?.runtimeProfile.reasoning, "xhigh");
  ...
  assert.deepEqual(card?.agent.agentCard?.domainTags, ["平台", "值班"]);
  assert.equal(card?.agent.agentCard?.currentFocus, "收口平台值班链路。");
  assert.equal(card?.agent.updatedAt, "2026-04-14T14:10:00.000Z");
...
test("PlatformControlPlaneService 会在默认建员时复用唯一 organization，并在多 organization 下沿用 supervisor 所属组织", () => {
  ...
  const primary = service.createAgent({
    ownerPrincipalId: "principal-platform-owner",
    agent: {
      departmentRole: "Platform",
      displayName: "平台负责人",
  ...
  assert.equal(primary.organization.organizationId, "org-platform");

  const extra = service.createAgent({
    ownerPrincipalId: "principal-platform-owner",
    agent: {
  ...
  assert.equal(extra.organization.organizationId, "org-extra");

  const inherited = service.createAgent({
    ownerPrincipalId: "principal-platform-owner",
    agent: {
      departmentRole: "Support",
      displayName: "平台跟班",
      mission: "沿用 supervisor 组织。",
      supervisorAgentId: primary.agent.agentId,
    },
  });
  assert.equal(inherited.organization.organizationId, "org-platform");
  assert.equal(inherited.agent.supervisorAgentId, primary.agent.agentId);
  ...
  assert.throws(
    ...
    /organizationId is required when owner has multiple organizations and no supervisorAgentId/i,
  );
});
